  ...
  const navLinks = [
    {
      label: "Portal",
      dropdown: [
        { href: "/subscribe", label: "Subscribe / Login" },
        { href: "/subscribe", label: "School Platform" },
        { href: "#", label: "Quiz App" },
        { href: "#", label: "Tuckshop" },
      ],
    },
                  ...
                  <div key={idx} className="relative group">
                    <button
                      className={`transition flex items-center gap-1 ${
                        pathname.startsWith(link.dropdown[0].href) || 
                        (link.dropdown[1] && pathname.startsWith(link.dropdown[1].href))
                    ...
                    <div className="absolute top-full left-0 mt-1 bg-white shadow-xl rounded-2xl border border-gray-100 py-2 min-w-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                      {link.dropdown.map((item, subIdx) => (
                        <Link
                          key={`${link.label}-${subIdx}`}
                          href={item.href}
                          className={`block px-6 py-2.5 text-sm hover:bg-gray-50 transition ${
                            pathname === item.href ? "text-[#C41230] font-semibold" : "text-gray-600"
                          }`}
                        >
                          {item.label}
                        </Link>
