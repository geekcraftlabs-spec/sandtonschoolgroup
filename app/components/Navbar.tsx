...
export default function Navbar() {
  ...
  // Show cart only on uniform shop pages
        ...
        <div className="h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6 text-sm font-medium">
            {navLinks.map((link, idx) => {
              if (link.dropdown) {
                const isActive = link.dropdown.some(item => pathname === item.href || pathname?.startsWith(item.href));
                return (
          ...
          {/* CTA Buttons + Cart Icon (Desktop) */}
          <div className="hidden md:flex items-center gap-3 shrink-0">
            {/* Cart Icon – only on uniform shop pages */}
            {showCart && (
        ...
        <div className="lg:hidden bg-white border-b border-gray-200 shadow-lg max-h-[80vh] overflow-y-auto">
          <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-gray-100">
              {/* Mobile cart link – only on uniform pages */}
              {showCart && (
                <Link
                  href="/cart"
                  onClick={() => setIsOpen(false)}
                ...
                >
                  🛒 Cart {itemCount > 0 && <span className="bg-[#C41230] text-white text-xs rounded-full px-2 py-1">{itemCount}</span>}
                </Link>
              )}
              <Link
                href="/apply"
